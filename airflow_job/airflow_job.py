...
with DAG(
    ...
    gcs_bucket = Variable.get("gcs_bucket", default_var="airflow-projetcs-self")
    bq_project = Variable.get("bq_project", default_var="bamboo-volt-453611-s5")
    ...
    batch_id = f"flight-booking-batch-{env}-{datetime.now().strftime('%Y%m%d%H%M%S')}-{uuid.uuid4()}"

    # Task 1: File Sensor for GCS
    file_sensor = GCSObjectExistenceSensor(
        task_id="check_file_arrival",
        bucket=gcs_bucket,
        object=f"flights_booking_data_cicd/source-{env}/flight_booking.csv",  # Full file path in GCS
    ...
    batch_details = {
        "pyspark_batch": {
            "main_python_file_uri": f"gs://{gcs_bucket}/flights_booking_data_cicd/spark-job/spark_transformation_job.py",  # Main Python file
        ...
        "environment_config": {
            "execution_config": {
                "service_account": "872814912354-compute@developer.gserviceaccount.com",
                "network_uri": "projects/bamboo-volt-453611-s5/global/networks/default",
                "subnetwork_uri": "projects/bamboo-volt-453611-s5/regions/us-central1/subnetworks/default",
            },
        },
    }
    ...
    pyspark_job = DataprocCreateBatchOperator(
        ...
        batch=batch_details,
        batch_id=batch_id,  # Use the unique batch ID
        gcp_conn_id="google_cloud_default",  # GCP connection
    )

    # Task Dependencies
    file_sensor >> pyspark_job  # Ensure the file sensor runs before the PySpark job